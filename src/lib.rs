...
impl Training {
	pub fn new() -> Self {
		...
		let mut model = vec![Expert{mean: vector![0., 0.], precision: vector![0.01, 0.01], ln_prior_weight: 0.}];
	...
	pub fn step(&mut self) {
		...
		for &d in &*data {
			for (i, &Expert{ln_prior_weight: c, precision : p, mean: µ}) in model.iter().enumerate() {
				...
				sum_grad_c[i] += 1.;
				let sq = vector![sq(d[0]-µ[0]), sq(d[1]-µ[1])];
				sum_grad_p[i] += -sq;
				sum_grad_µ[i] += vector![2.*sq[0]*µ[0], 2.*sq[1]*µ[1]];
			}
			...
			for _ in 0..10 {
				...
				d_hat = MultivariateNormal::new_from_nalgebra(vector![sum_pµ[0]/sum_p[0], sum_pµ[1]/sum_p[1]], matrix![1./sqrt(sum_p[0]), 0.; 0., 1./sqrt(sum_p[1])]).unwrap().sample(rng);
			...
			for (i, &Expert{mean: µ, precision : p, ln_prior_weight: c}) in model.iter().enumerate() {
				...
				if rng.random_bool(1./(1.+q)) { continue; } // uniform
				sum_grad_c[i] -= 1.;
				let sq = vector![sq(d_hat[0]-µ[0]), sq(d_hat[1]-µ[1])];
				sum_grad_p[i] -= -sq;
				sum_grad_µ[i] -= vector![2.*sq[0]*µ[0], 2.*sq[1]*µ[1]];
			}
		}
